                    ...
                    <li class="mui-table-view-cell" v-for="item in items">
                        <div class="mui-table"  v-bind:class="{expend:item.je<0}">
                            <div class="mui-table-cell mui-col-xs-10">
                                <h5 class="mui-ellipsis">消费时间：{{ item.createtime }}</h5>
                                <h5>店名：{{item.mdmc}}</h5>
                                <h5>单号：{{item.ordercode}} </h5>
                                <h5>金额：{{item.je}}</h5>
                            </div>

                            <div class="mui-table-cell mui-col-xs-2 " style="text-align: center;">
                                <span class="mui-h5">余额{{item.ye}}</span>
                            </div>
                        </div>
                    </li>
                </ul>
...
<script type="text/javascript">
    ...
    function getVipHistory() {
        mui.ajax({
            url: '/wxsubscribe/getVipHistoryListByVipCard/' + document.getElementById("vipcard").innerHTML,
